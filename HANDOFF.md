...
> - **Single-click links (roadmap #6)** — the blue `#number` opens the PR and the `ENA-####` tag opens its linked issue (uses `BoardRow.issue_url`, already in core) on a single click, with hover-underline + pointer. Row double-click still opens the PR.
